  ...
  const fetchOverviewData = async () => {
    try {
      ...
      const [customersRes, deliveriesRes, restaurantsRes, paymentsRes] =
        await Promise.all([
          axios.get("http://localhost:3003/api/admin/users?role=customer", {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get("http://localhost:3003/api/admin/users?role=delivery", {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get("http://localhost:3003/api/admin/restaurants", {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get("http://localhost:3003/api/admin/payments", {
            headers: { Authorization: `Bearer ${token}` }
          })
        ]);
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
              ...
              {kpiCard(
                "Total Revenue",
                `Rs.${totals.totalRevenue.toFixed(2)}`,
                <Wallet size={40} />,
                "text-yellow-600",
                renderTrend(
                  totals.totalRevenue,
