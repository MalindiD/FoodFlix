...
function Dashboard() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const RESTAURANT_API =
        process.env.REACT_APP_RESTAURANT_API || "http://localhost:3003";
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      ...
      const response = await fetch(
        `${RESTAURANT_API}/restaurants/${restaurantId}/orders/${orderId}/status`,
        {
          method: "PATCH",
          headers: {
  ...
  // Every order: set paymentStatus to 'completed'
  const ordersWithCompletedPayments = orders.map((order) => ({
    ...order,
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            <div>
              ...
              {renderOrdersTable(
                ordersWithCompletedPayments.filter(
                  (order) => order.status?.toLowerCase() === "pending"
                ),
                handleUpdateStatus,
                formatDate,
