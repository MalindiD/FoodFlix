...
const OrderTracking = () => {
  ...
  useEffect(() => {
    const fetchOrderStatus = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/orders/tracking/public/${orderId}`);
  ...
  useEffect(() => {
    const fetchDeliveryDetails = async () => {
      try {
        const res = await axios.get(`http://localhost:5003/api/delivery/public/${orderId}`);
  ...
  const currentStep = statusSteps.findIndex((step) => step.label === orderStatus);

  if (loading || !deliveryDetails) {
    return <p className="text-gray-500 text-lg mt-20 text-center">Loading delivery information...</p>;
  }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center px-2 pt-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-2xl mb-8 text-left">
        <h3 className="text-xl font-bold text-orange-500 mb-4">Order Details</h3>
        <div className="text-gray-700 mb-4">
          <p><strong>Order ID:</strong> {orderId}</p>
          <p><strong>Status:</strong> {orderStatus}</p>
        </div>

        <h3 className="text-xl font-bold text-orange-500 mb-4 mt-6">Delivery Partner</h3>
        <div className="flex items-center gap-4">
          ...
          <div className="text-gray-700">
            ...
            {driverLocation && (
              <p><strong>Current Location:</strong> {driverLocation.lat.toFixed(5)}, {driverLocation.lng.toFixed(5)}</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* Open Mock Driver Button */}
        <a
        href={`/mock-driver/${orderId}/${driverLocation?.lat || 6.9157}/${driverLocation?.lng || 79.863}`}
        target="_blank"
